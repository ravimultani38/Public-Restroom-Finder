...
const Map = ({ setPopupContent, setShowPopup, setPopupPosition }) => {
    ...
    const fetchBathrooms = async () => {
      try {
        ...
        const hardcodedBathrooms = [
          { coord: fromLonLat([-74.006, 40.7128]), name: "New York City, USA" },
          {
            coord: fromLonLat([-73.975189, 40.715899]),
            name: "East River Park Zone 3",
            hours: "8am-4pm, Open later seasonally",
          },
          {
            coord: fromLonLat([-73.843252, 40.751662]),
            name: "Passerelle Building",
            hours: "8am-4pm, Open later seasonally",
          },
          {
            coord: fromLonLat([-74.006873, 40.743643]),
            name: "The High Line Zone 1",
            hours: "Year Round",
          },
        ];
  ...
  useEffect(() => {
    const mapObject = new OlMap({
      ...
      view: new View({
        center: fromLonLat([-74.006, 40.7128]), // Centering the map around NYC
        zoom: 12,
      }),
    });
  ...
  useEffect(() => {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          const closest = [...bathrooms]
            .map((bathroom) => ({
              ...bathroom,
              distance: calculateDistance(
                userCoordinates,
                toLonLat(bathroom.coord)
              ),
            }))
    ...
    map.on("singleclick", (evt) => {
      ...
      if (feature) {
        ...
        setPopupContent(
          `${name}<br>Latitude: ${latLon[1].toFixed(
            4
          )}<br>Longitude: ${latLon[0].toFixed(4)}<br>Hours: ${hours}`
        );
        setShowPopup(true);
        ...
        <ul>
          {closestBathrooms.map((bathroom, index) => (
            <li key={index}>
              ...
              Hours: {bathroom.hours}
              <br />
              Distance: {bathroom.distance.toFixed(2)} miles
              <br />
              {console.log(toLonLat(bathroom.coord))}
              <a className="btn btn-primary"
                href={
                  "https://www.google.com/maps/dir/?api=1&destination=" +
                  toLonLat(bathroom.coord).reverse().join(",")
                }
                target="_blank"
